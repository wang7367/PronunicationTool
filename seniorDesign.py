...
HMONG_TONES = ["_", "B", "G", "J", "M", "S", "V"]

MAND_WORDS = ["shuffle"] + headerfile.mandWords
VIET_WORDS = [["shuffle"] + headerfile.vietWordsLevel, ["shuffle"] + headerfile.vietWordsDeep,
              ["shuffle"] + headerfile.vietWordsSharp, ["shuffle"] + headerfile.vietWordsHeavy,
              ["shuffle"] + headerfile.vietWordsAsking, ["shuffle"] + headerfile.vietWordsTumbling]
HMONG_WORDS = ["shuffle"] + headerfile.hmongWords
...
class ToneLearner:
    def __init__(self, master):
        ...
        self.master.title("Learning Tonal Languages")
        ...
        self.last_tone = "Select language"
        self.current_word = "Select language"
        self.last_voice = "Select language"
        ...
        # Colored text boxes
        self.score_output, self.score_output_label = self.create_canvas(f"", (5, 1), bg=skyblue)
        self.predicted_output, self.predicted_output_label = self.create_canvas(f"", (4, 1), bg=skyblue)
        # Dropdown buttons
    ...
    def language_changed(self, *args):
        ...
        self.language_button.config(bg=skyblue)
        self.searchbar_bg.config(bg=skyblue)
        ...
        if self.language.get() == "Mandarin":
            ...
            self.word = self.search_bar.get_trace()
            self.word.trace("w", self.word_changed)
        elif self.language.get() == "Vietnamese":
            # Remake with Vietnamese tones/voices; clear the Word dropdown, this varies by tone
            self.tone_button, self.tone = self.create_dropdown((1, 1), options=VIET_TONES, callback=self.tone_changed,
                                                               bg=highlighter, default="Select Tone")
            self.search_bar.des()
            self.search_bar_frame = tk.Frame(self.master, relief=tk.GROOVE)
            self.search_bar_frame.grid(row=2, column=1)
            ...
            self.word = self.search_bar.get_trace()
            self.word.trace("w", self.word_changed)
        elif self.language.get() == "Hmong":
            self.tone_button, self.tone = self.create_dropdown((1, 1), options=HMONG_TONES, callback=self.tone_changed,
                                                               bg=highlighter, default="Select Tone")
            self.search_bar.des()
            self.search_bar_frame = tk.Frame(self.master, relief=tk.GROOVE)
            self.search_bar_frame.grid(row=2, column=1)
            self.search_bar = self.SearchBar(self.search_bar_frame, HMONG_WORDS, self.button_font)
            self.word = self.search_bar.get_trace()
            self.word.trace("w", self.word_changed)
    ...
    def tone_changed(self, *args):
        ...
        self.tone_button.config(bg=skyblue)
        if self.language.get() == "Mandarin":
            if self.word.get() == "":
            ...
            self.search_bar_frame = tk.Frame(self.master, relief=tk.GROOVE)
            self.search_bar_frame.grid(row=2, column=1)
            self.search_bar = self.SearchBar(self.search_bar_frame, VIET_WORDS[viet_tone[self.tone.get()]],
                                             self.button_font)
            self.word = self.search_bar.get_trace()
            self.word.trace("w", self.word_changed)
        elif self.language.get() == "Hmong":
            if self.word == "":
                ...
                self.current_word_meaning_canvas.itemconfig(self.current_word_meaning_label,
                                                            text=self.pinyin_to_char())
    ...
    def word_changed(self, *args):
        ...
        self.searchbar_bg.config(bg=skyblue)
        self.current_word_canvas.itemconfig(self.current_word_label, text=str(self.current_word) + " " + self.tone.get())
        if self.current_word == "shuffle":
            self.record_button.destroy()
            self.play_button.destroy()
            self.play_button = self.create_button("Play", (2, 2), span=(1, 2), command=lambda: self.play())
            self.record_button = self.create_button("Record", (2, 4), span=(1, 2), command=lambda: self.record())
            self.next_button = self.create_button("Next", (2, 6), span=(1, 2), command=lambda: self.random_word())
            self.random_word()
        else:
            if self.language.get() == "Mandarin":
                self.current_word_meaning_canvas.itemconfig(self.current_word_meaning_label, text=self.pinyin_to_char())
    ...
    def create_canvas(self, text, coordinates, span=(1, 1), bg=skyblue, relief=tk.RAISED, font=None, size=(40, 250)):
    ...
    def create_dropdown(self, coordinates, options=("",), callback=None,  span=(1, 1), bg=skyblue, relief=tk.RAISED,
    ...
    def get_audio_file(self):
        # sound = "Hmong_MP3/B Tone/aab.MP3"
         if self.language.get() == "Mandarin":
             ...
             sound = "VietnameseMP3s/" + self.tone.get().lower() + "_" \
                     + self.current_word.encode('ascii', 'xmlcharrefreplace').decode() + ".mp3"
         elif self.language.get() == "Hmong":
              sound = "a.mp3"
              if self.tone.get() == "_":
                  sound = "HmongMP3s/" + self.current_word + ".mp3"
              else:
                  sound = "HmongMP3s/" + self.current_word + self.tone.get().lower() + ".mp3"
         else:
    ...
    def update_score(self, score):
        ...
        if self.language.get() == "Mandarin":
            ...
            predicted_tone = vdl.viet_deepL("user.mp3")
            self.predicted_output.itemconfig(self.predicted_output_label, text=VIET_TONES[predicted_tone[0] - 1] + " Tone")
            if VIET_TONES[predicted_tone[0] - 1] == self.tone.get():
                bg = "green"
            else:
                bg = "red"
            self.predicted_output.config(bg=bg)
            ...
            pass
            # TODO
            # predicted_tone = hdl.hmong_deepL("user.mp3")
            # self.predicted_output.itemconfig(self.predicted_output_label, text=HMONG_TONES[predicted_tone[0] - 1] + " Tone")
            # if HMONG_TONES[predicted_tone[0] - 1] == self.tone.get():
            #     bg = "green"
            # else:
            #     bg = "red"
            # self.predicted_output.config(bg=bg)
    ...
    # Sets the current word to a randomly selected word with the current language, tone, and voice
    def random_word(self):
        if self.language.get() == "Mandarin":
            # Start RNG at 1 since "shuffle" = 0
            i = random.randrange(1, len(MAND_WORDS), 1)
            self.current_word = MAND_WORDS[i]
        elif self.language.get() == "Vietnamese":
            i = random.randrange(1, len(VIET_WORDS[viet_tone[self.tone.get()]]), 1)
            self.current_word = VIET_WORDS[viet_tone[self.tone.get()]][i]
        elif self.language.get() == "Hmong":
            i = random.randrange(1, len(HMONG_WORDS), 1)
            self.current_word = HMONG_WORDS[i]
        self.current_word_canvas.itemconfig(self.current_word_label,
                                            text=str(self.current_word) + " " + self.tone.get())
        self.current_word_meaning_canvas.itemconfig(self.current_word_meaning_label,
                                                    text=self.pinyin_to_char())
